...
void draw_game(void)
{
	...
	if (pausemode)
	{
		...
		if (gameMode)
			spFontDrawMiddle(screen->w>>1,(screen->h>>1)-font->maxheight*9/2,-1,"Mode: Hard",font);
		else
			spFontDrawMiddle(screen->w>>1,(screen->h>>1)-font->maxheight*9/2,-1,"Mode: Easy",font);
		sprintf(buffer,"Volume: %i %%",volume*100/2048);
		spFontDrawMiddle(screen->w>>1,(screen->h>>1)-font->maxheight*7/2,-1,buffer,font);
		sprintf(buffer,"Music volume: %i %%",volumefactor*100/2048);
		spFontDrawMiddle(screen->w>>1,(screen->h>>1)-font->maxheight*5/2,-1,buffer,font);
		spFontDrawMiddle(screen->w>>1,(screen->h>>1)-font->maxheight*3/2,-1,"Return",font);
...
int calc_game(Uint32 steps)
{
	...
	if (pausemode)
	{
		...
		{
			case 0:
				if (engineInput->axis[0] != 0 || engineInput->button[SP_BUTTON_LEFT] || engineInput->button[SP_BUTTON_RIGHT])
				{
